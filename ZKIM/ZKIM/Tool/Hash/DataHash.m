...
// Constants
static const size_t DataHashDefaultChunkSizeForReadingData = 4096; // in bytes

// Function pointer types for functions used in the computation
// of a cryptographic hash.
typedef int (*DataHashInitFunction)   (uint8_t *hashObjectPointer[]);
typedef int (*DataHashUpdateFunction) (uint8_t *hashObjectPointer[], const void *data, CC_LONG len);
typedef int (*DataHashFinalFunction)  (unsigned char *md, uint8_t *hashObjectPointer[]);

// Structure used to describe a hash computation context.
typedef struct _DataHashComputationContext {
    DataHashInitFunction initFunction;
    DataHashUpdateFunction updateFunction;
    DataHashFinalFunction finalFunction;
    size_t digestLength;
    uint8_t **hashObjectPointer;
} DataHashComputationContext;

#define MAX_READ_STREAM_STATUS_POLLING_ATTEMPTS 10

#define DataHashComputationContextInitialize(context, hashAlgorithmName)                    \
CC_##hashAlgorithmName##_CTX hashObjectFor##hashAlgorithmName;                          \
context.initFunction      = (DataHashInitFunction)&CC_##hashAlgorithmName##_Init;       \
context.updateFunction    = (DataHashUpdateFunction)&CC_##hashAlgorithmName##_Update;   \
context.finalFunction     = (DataHashFinalFunction)&CC_##hashAlgorithmName##_Final;     \
...
@implementation DataHash

#pragma mark -
#pragma mark private class helpers
+ (NSString *)hashOfFileAtPath:(NSString *)filePath withComputationContext:(DataHashComputationContext *)context {
    NSString *result = nil;
    ...
    const size_t chunkSizeForReadingData = DataHashDefaultChunkSizeForReadingData;
...
+ (NSString *)hashOfALAssetRepresentation:(ALAssetRepresentation *)assetRepresentation withComputationContext:(DataHashComputationContext *)context {
...
+ (NSString *)hashOfNSData:(NSData *)data withComputationContext:(DataHashComputationContext *)context {
    ...
    if(DataHashDefaultChunkSizeForReadingData > data.length) {
        chunkSizeForReadingData = data.length;
    } else {
        chunkSizeForReadingData = DataHashDefaultChunkSizeForReadingData;
    }
...
+ (NSString *)md5HashOfData:(NSData *)data {
    DataHashComputationContext context;
    DataHashComputationContextInitialize(context, MD5);
    return [self hashOfNSData:data withComputationContext:&context];
}

+ (NSString *)sha1HashOfData:(NSData *)data {
    DataHashComputationContext context;
    DataHashComputationContextInitialize(context, SHA1);
    return [self hashOfNSData:data withComputationContext:&context];
}

+ (NSString *)sha512HashOfData:(NSData *)data {
    DataHashComputationContext context;
    DataHashComputationContextInitialize(context, SHA512);
    return [self hashOfNSData:data withComputationContext:&context];
}

+ (NSString *)crc32HashOfData:(NSData *)data {
    DataHashComputationContext context;
    DataHashComputationContextInitialize(context, CRC32);
    return [self hashOfNSData:data withComputationContext:&context];
}
...
@implementation NSData (DataHash)

- (NSString *)md5 {
    return [DataHash md5HashOfData:self];
}

- (NSString *)sha1 {
    return [DataHash sha1HashOfData:self];
}

- (NSString *)sha512 {
    return [DataHash sha512HashOfData:self];
}

- (NSString *)crc32 {
    return [DataHash crc32HashOfData:self];
}

@end

@implementation NSString (DataHash)

- (NSString *)md5 {
    return [DataHash md5HashOfData:[self dataUsingEncoding:NSUTF8StringEncoding]];
}

- (NSString *)sha1 {
    return [DataHash sha1HashOfData:[self dataUsingEncoding:NSUTF8StringEncoding]];
}

- (NSString *)sha512 {
    return [DataHash sha512HashOfData:[self dataUsingEncoding:NSUTF8StringEncoding]];
}

- (NSString *)crc32 {
    return [DataHash crc32HashOfData:[self dataUsingEncoding:NSUTF8StringEncoding]];
}

@end

@implementation NSFileManager (DataHash)

+ (NSString *)fileMD5AtPath:(NSString *)filePath {
    return [DataHash md5HashOfFileAtPath:filePath];
}

+ (NSString *)fileSHA1AtPath:(NSString *)filePath {
    return [DataHash sha1HashOfFileAtPath:filePath];
}

+ (NSString *)fileSHA512AtPath:(NSString *)filePath {
    return [DataHash sha512HashOfFileAtPath:filePath];
}

+ (NSString *)fileCRC32AtPath:(NSString *)filePath {
    return [DataHash crc32HashOfFileAtPath:filePath];
}

@end

@implementation ALAssetRepresentation (DataHash)

- (NSString *)md5 {
    return [DataHash md5HashOfALAssetRepresentation:self];
}

- (NSString *)sha1 {
    return [DataHash sha1HashOfALAssetRepresentation:self];
}

- (NSString *)sha512 {
    return [DataHash sha512HashOfALAssetRepresentation:self];
}

- (NSString *)crc32 {
    return [DataHash crc32HashOfALAssetRepresentation:self];
}
